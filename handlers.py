...
async def cmd_start(message: Message):
    ...
    await message.answer("""   
                ✨  Добро пожаловать в сервис быстрого резюмирования!
    
    💡  Как получить краткое содержание?

    📑  Просто отправьте мне:
        * Текст напрямую
        * Или изображение с текстом
    
    👉 Пошаговая инструкция:
        1. Нажмите на значок прикрепления 
        2. Выберите раздел фото или видео (галерея)
        3. Откройте изображение
        4. Включить сжатие изображения (если функция есть)
                        """)
...
async def cmd_message(message: Message):
    if message.text is not None:
        ...
        await message.answer('Формат документа не принимается. Отправьте текст или изображение с текстом (значок '
                             'прикрепления -> фото или видео (галерея) -> выбрать изображение -> сжать изображение (если функция есть)')
